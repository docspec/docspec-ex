defmodule DocSpec.Spec.TableHeader do
  @moduledoc """
  A header cell within a table row. Equivalent to HTML `<th>` element.

  ## Fields

  * `abbreviation` - A short, abbreviated description of the header cell's content provided as an alternative label.
  * `scope` - Indicates whether this header applies to a column or row.
  * `rowspan` - Represents the number of rows this table header must span.
  * `colspan` - Represents the number of columns this table header must span.
  ...
  * `id` (required)
  * `type` - Always `"https://alpha.docspec.io/TableHeader"`.
  """

  use DocSpec.Spec.Schema, type: "https://alpha.docspec.io/TableHeader"

  typed_embedded_schema null: false do
    # A short, abbreviated description of the header cell's content provided as an alternative label.
    field :abbreviation, :string
    # Indicates whether this header applies to a column or row.
    field :scope, :string
    # Represents the number of rows this table header must span.
    field :rowspan, :integer, default: 1
    # Represents the number of columns this table header must span.
  ...
  def changeset(schema, attrs) do
    schema
    |> cast(attrs, [:abbreviation, :scope, :rowspan, :colspan, :id, :type], @cast_opts)
    ...
    |> validate_uuid()
    |> validate_inclusion(:scope, ["column", "row"])
  end
end
