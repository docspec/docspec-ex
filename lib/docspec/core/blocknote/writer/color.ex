defmodule DocSpec.Core.BlockNote.Writer.Color do
  @moduledoc """
  ...
  alias DocSpec.Util.Color.RGB.Hex
  ...
  ## Examples
      ...
      iex> DocSpec.Core.BlockNote.Writer.Color.nearest(:text, "#d9730d")
      {:ok, "orange"}

      iex> DocSpec.Core.BlockNote.Writer.Color.nearest(:background, "#d9730d")
      {:ok, "orange"}

      iex> DocSpec.Core.BlockNote.Writer.Color.nearest(:text, "#64473a")
      {:ok, "brown"}

      iex> DocSpec.Core.BlockNote.Writer.Color.nearest(:background, "#64473a")
      {:ok, "brown"}

      iex> DocSpec.Core.BlockNote.Writer.Color.nearest(:text, {224, 62, 62})
      {:ok, "red"}

      iex> DocSpec.Core.BlockNote.Writer.Color.nearest(:background, {224, 62, 62})
      {:ok, "orange"}

      iex> DocSpec.Core.BlockNote.Writer.Color.nearest(:text, "not-a-color")
      {:error, :invalid}

      iex> DocSpec.Core.BlockNote.Writer.Color.nearest(:background, "not-a-color")
      {:error, :invalid}
  """
  ...
  def nearest(type, color) when is_binary(color) and (type == :background or type == :text) do
    with {:ok, rgb} <- Hex.to_rgb(color) do
      nearest(type, rgb)
    end
  end
